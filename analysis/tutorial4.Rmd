...
We will add five fields to the fracture dataset:  charge_status, practicioner, service_year, age, and age_group_10 (10 year age group).  The data flows through a pipe, going from fracture, mutating into another variable, and then going back to fracture.  The mutations are done using transformation functions,  including: `ifelse`, `substring`, `as.integer`, and `cut`. 

The function `ifelse` takes 3 arguments:  a condition, the return value if the condition is true, and the return value if the condition is false.   It is useful for creating categorical variables.  

The `cut` function slices a continuous variable into chunks at the position of designated breaks, in this case, 10 year age groups.
l;
...
2. Create another field in `fracture` to hold 20 year age groups.  

# Visualise and clean the fractures dataset

What is the distribution of the `fee charged` field.  
...
What gender gets ankle fractures?  The first step is to plot the data.
...
There is a difference between the number of men and women who get ankle fractures.  Is this difference statistically significant?

The chi squared test tells us if this difference is statistically significant. We will pipe fracture into the `table` function, which counts the number of levels of each variable.  
...
```
`Prop.table` calculates the proportions for each variable (like percentages).  This standardises the value, putting them on the same base
...
The chi squared test above tells us that this difference, between men and women, is not due to chance.
...
ggplot( fracture, mapping=aes(fill=practicioner, x=charge_status) ) + 
  geom_bar( ) 

```
```{r chisq_chargestatus_practicioner}
...
Is the age range of people who break their ankle different for the different genders?  We first look at the data.
...
```
Again, because the Pvalue is less than .05, the different is unlikely due to chance.  
...
ggplot( fracture, mapping=aes(x=age, fill=practicioner) )  + 
  geom_bar( ) 

```


```{r t.test_age_practicioner}

t.test( age~practicioner , data=fracture)
...
ggplot( fracture, mapping=aes(x=age, fill=charge_status) )  +
  geom_bar( ) 
...
t.test( age~charge_status, data=fracture)

```

# Modelling - is the price increasing over time?

Is the incidence of overcharging increasing over time?  To determine this, we will count the number of overcharges per year, and plot them.  Note, when we use `ggplot` in a pipe, we don't need to include the dataset, because the dataset is piped in.
...
The model says that the model is not a linear trend.
...
3. Choose 2 versions of your disease.  Is there a significant difference between the length of stay (LOS) for these 2 versions?
